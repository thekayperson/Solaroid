...
HXLINE( 113)
HXLINE( 114)		HX_VARI( Float,dx) = ((SpriteB->x + SpriteB->origin->x) - (SpriteA->x + SpriteA->origin->x));
HXLINE( 115)		HX_VARI( Float,dy) = ((SpriteB->y + SpriteB->origin->y) - (SpriteA->y + SpriteA->origin->y));
...
Float FlxAngle_obj::angleBetweenPoint( ::flixel::FlxSprite Sprite, ::flixel::math::FlxPoint Target,::hx::Null< bool >  __o_AsDegrees){
            	...
            	HX_STACK_ARG(AsDegrees,"AsDegrees")
HXLINE( 133)
HXLINE( 134)		HX_VARI( Float,dx) = (Target->x - (Sprite->x + Sprite->origin->x));
HXLINE( 135)		HX_VARI( Float,dy) = (Target->y - (Sprite->y + Sprite->origin->y));
...
HXLINE( 160)		HX_VARI(  ::flixel::math::FlxPoint,p) = Object->getScreenPosition(null(),null());
HXLINE( 162)		HX_VARI( Float,dx) = (( (Float)(::flixel::FlxG_obj::mouse->screenX) ) - p->x);
HXLINE( 163)		HX_VARI( Float,dy) = (( (Float)(::flixel::FlxG_obj::mouse->screenY) ) - p->y);
HXLINE( 165)		p->put();
...
HXLINE( 210)				degrees = 90;
            			...
            			break;
            			default:{
HXLINE( 216)				HX_VARI( int,f) = Facing;
HXDLIN( 216)				if ((f == (256 | 1))) {
HXLINE( 210)					degrees = -135;
            				}
            				else {
HXLINE( 217)					HX_VARI( int,f) = Facing;
HXDLIN( 217)					if ((f == (256 | 16))) {
HXLINE( 210)						degrees = -45;
            					}
            					else {
HXLINE( 218)						HX_VARI( int,f) = Facing;
HXDLIN( 218)						if ((f == (4096 | 1))) {
HXLINE( 210)							degrees = 135;
            						}
            						else {
HXLINE( 219)							HX_VARI( int,f) = Facing;
...
HXLINE( 234)
HXLINE( 235)		HX_VARI(  ::flixel::math::FlxPoint,p) = point;
...
STATIC_HX_DEFINE_DYNAMIC_FUNC3(FlxAngle_obj,getCartesianCoords,return )
 ...
 ::flixel::math::FlxPoint FlxAngle_obj::getPolarCoords(Float X,Float Y, ::flixel::math::FlxPoint point){
            	...
            	HX_STACK_ARG(point,"point")
HXLINE( 253)
HXLINE( 254)		HX_VARI(  ::flixel::math::FlxPoint,p) = point;
