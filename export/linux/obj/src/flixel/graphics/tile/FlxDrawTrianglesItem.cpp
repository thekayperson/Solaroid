...
HXLINE(  59)			HX_VARI(  ::openfl::display::Graphics,gfx) = camera->debugLayer->get_graphics();
...
HXLINE( 101)		HX_VARI( int,verticesLength) = vertices->get_length();
HXLINE( 102)		HX_VARI( int,prevVerticesLength) = this->vertices->get_length();
HXLINE( 103)		HX_VARI( int,numberOfVertices) = ::Std_obj::_hx_int((( (Float)(verticesLength) ) / ( (Float)(2) )));
HXLINE( 104)		HX_VARI( int,prevIndicesLength) = this->indices->get_length();
HXLINE( 105)		HX_VARI( int,prevUVTDataLength) = this->uvtData->get_length();
HXLINE( 106)		HX_VARI( int,prevColorsLength) = this->colors->get_length();
HXLINE( 107)		HX_VARI( int,prevNumberOfVertices) = this->get_numVertices();
HXLINE( 109)		HX_VAR( Float,tempX);
HXDLIN( 109)		HX_VAR( Float,tempY);
HXLINE( 110)		HX_VARI( int,i) = 0;
HXLINE( 111)		HX_VARI( int,currentVertexPosition) = prevVerticesLength;
...
HXLINE( 139)			HX_VARI( int,uvtDataLength) = uvtData->get_length();
HXLINE( 140)			{
HXLINE( 140)				int _g = 0;
...
HXLINE( 197)
HXLINE( 198)		HX_VARI( int,prevVerticesPos) = this->verticesPosition;
HXLINE( 199)		HX_VARI( int,prevIndicesPos) = this->indicesPosition;
HXLINE( 200)		HX_VARI( int,prevColorsPos) = this->colorsPosition;
HXLINE( 201)		HX_VARI( int,prevNumberOfVertices) = this->get_numVertices();
...
HXDLIN( 203)		HX_VARI(  ::flixel::math::FlxPoint,point1) = point;
HXLINE( 204)		{
...
HXLINE( 250)			HX_VARI( Float,red) = ((Float)1.0);
HXLINE( 251)			HX_VARI( Float,green) = ((Float)1.0);
HXLINE( 252)			HX_VARI( Float,blue) = ((Float)1.0);
HXLINE( 253)			HX_VARI( Float,alpha) = ((Float)1.0);
...
HXDLIN( 266)			HX_VARI( int,color5) = color;
