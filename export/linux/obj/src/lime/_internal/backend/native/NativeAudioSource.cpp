...
HXLINE(  56)						HX_VARI(  ::Dynamic,buffer) = _g1->__get(_g);
HXDLIN(  56)						_g = (_g + 1);
...
HXLINE(  98)			HX_VARI(  ::lime::media::vorbis::VorbisFile,vorbisFile) = this->parent->buffer->_hx___srcVorbisFile;
...
HXDLIN( 210)		HX_VARI(  ::lime::utils::ArrayBufferView,buffer1) = this1;
HXLINE( 211)		HX_VARI( int,read) = 0;
HXDLIN( 211)		HX_VARI( int,total) = 0;
HXDLIN( 211)		HX_VAR( int,readMax);
...
HXLINE( 241)
HXLINE( 243)		HX_VARI(  ::lime::media::vorbis::VorbisFile,vorbisFile) = null();
HXLINE( 244)		HX_VARI( int,position) = 0;
HXLINE( 246)		if (::hx::IsNull( buffers )) {
HXLINE( 248)			HX_VARI( int,buffersProcessed) = ( (int)(::lime::media::openal::AL_obj::getSourcei(this->handle,4118)) );
...
HXLINE( 270)			HX_VARI( int,numBuffers) = 0;
HXLINE( 271)			HX_VAR(  ::lime::utils::ArrayBufferView,data);
HXLINE( 273)			{
HXLINE( 273)				int _g = 0;
HXDLIN( 273)				while((_g < buffers->get_length())){
HXLINE( 273)					HX_VARI(  ::Dynamic,buffer) = buffers->__get(_g);
HXDLIN( 273)					_g = (_g + 1);
...
HXDLIN( 352)					HX_VARI( int,time2) = (time1 - this->parent->offset);
...
HXLINE( 358)					HX_VARI( int,offset) = ( (int)(::lime::media::openal::AL_obj::getSourcei(this->handle,4134)) );
HXLINE( 359)					HX_VARI( Float,ratio) = (( (Float)(offset) ) / ( (Float)(this->dataLength) ));
HXLINE( 360)					HX_VARI( Float,totalSeconds) = (( (Float)(this->samples) ) / ( (Float)(this->parent->buffer->sampleRate) ));
HXLINE( 362)					int time = ::Std_obj::_hx_int(((totalSeconds * ratio) * ( (Float)(1000) )));
HXDLIN( 362)					HX_VARI( int,time1) = (time - this->parent->offset);
...
HXLINE( 393)					HX_VARI( Float,secondOffset) = (( (Float)((value + this->parent->offset)) ) / ( (Float)(1000) ));
HXLINE( 394)					HX_VARI( Float,totalSeconds) = (( (Float)(this->samples) ) / ( (Float)(this->parent->buffer->sampleRate) ));
...
HXLINE( 399)					HX_VARI( Float,ratio) = (secondOffset / totalSeconds);
HXLINE( 400)					HX_VARI( int,totalOffset) = ::Std_obj::_hx_int((( (Float)(this->dataLength) ) * ratio));
...
HXLINE( 413)			HX_VARI( int,timeRemaining) = (this->getLength() - value);
...
HXLINE( 472)			HX_VARI( int,timeRemaining) = (value - this->getCurrentTime());
...
HXLINE( 499)			HX_VARI( ::Array< Float >,value) = ::lime::media::openal::AL_obj::getSource3f(this->handle,4100);
